...
tempData.mainEndPoint = "/api/users";
...
async function loadModule(permissionStr) {
	...
	const response = await Request.send("/api/regexes/USER");
...
const getInitialTableData = async () => {
	...
	const response = await Request.send(
		`${tempData.mainEndPoint}/search/ /skip/0`,
		"GET"
	);
...
const searchEntries = async (searchValue) => {
	...
	const response = await Request.send(
		`${tempData.mainEndPoint}/search/${searchValue}/skip/0`,
		"GET"
	);
...
const loadMoreEntries = async (searchValue, rowsCount) => {
	...
	const response = await Request.send(
		`${tempData.mainEndPoint}/search/${searchValue}/skip/${rowsCount}`,
		"GET"
	);
...
const getTableData = (responseData) => {
	...
	return responseData.map((entry) => {
		const roles = entry.userRoles.map((ur) => ur.role.name);
		return {
			entryId: entry.id,
			"E-Mail": entry.email,
...
const addEntry = async () => {
	...
	// get response
	const response = await Request.send(tempData.mainEndPoint, "POST", {
		data: data,
	});
...
const editEntry = async (id = mainTable.selectedEntryId) => {
	...
	FormUtil.selectDropdownOptionByValue("isActive", entry.isActive);
...
const updateEntry = async () => {
	...
	for (let key in newEntryObj) {
		...
		try {
			tempData.selectedEntry[key] =
				tempData.selectedEntry[key] == null ? "" : tempData.selectedEntry[key];
	...
	const response = await Request.send(
		`${tempData.mainEndPoint}/${newEntryObj.id}`,
		"PUT",
		{ data: newEntryObj }
	);
